...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def _get_resized_embeddings(
        ...
        complex_value = torch.load('././' + dataset + '/t5_complex_model.tar')
        self.adapter = nn.Linear(1536, old_embedding_dim)
        add_embedding = self.adapter(complex_value['ent_embed'])
        n_old = new_num_tokens - add_embedding.shape[0]
        print(n_old)
        new_embeddings.weight.data[:n_old, :] = old_embeddings.weight.data[:n_old, :]  # 前行转移过去
        new_embeddings.weight.data[n_old:, :] = add_embedding
