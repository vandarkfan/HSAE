...
def read_file(configs, dataset_path, dataset, filename, file_cluster,mode='descrip'):
    ...
    if file_cluster!=None:
        ...
        clu3fen = np.load('complex_wn18rr1536/entity_classifications.npy',allow_pickle=True).item()
    file_name = os.path.join(dataset_path, dataset, filename)
    ...
    for i in range(1, len(lines)):
        ...
        if file_cluster!=None:
            name = name + ', '+str(int(cluster[int(ids)])) + ' ' + str(clu3fen[i-1][0])
        else:
            name = name
        #if file_cluster!=None:
        #    name = str(int(cluster[int(ids)])) + ' ' + str(clu3fen[i-1][0]) + ' ' + name
        #else:
        #    name = name
        if mode == 'descrip':
            name = name.split(' ')
...
def get_next_token_dict(configs, ent_token_ids_in_trie, prefix_trie):
    ...
    for ent_id in tqdm(range(configs.n_ent)):
        ...
        # sparse_mask = sp.coo_matrix(([1] * len(rows), (rows, cols)), shape=(len(input_ids), configs.vocab_size + configs.n_ent))
        # neg_candidate_mask.append(sparse_mask)
    # return neg_candidate_mask, next_token_dict
    return None, next_token_dict
